...
parseTable xs = map words (lines xs)
\end{code}
...
sameNumCol :: Table -> Bool
sameNumCol (x:[]) = True
sameNumCol (x:x2:xs)
    | length x == length x2 = sameNumCol(x2:xs)
    | otherwise = False

validTable :: Table -> Bool
validTable (t:ts)
    | length (t:ts) < 2 = False
    | sameNumCol (t:ts) == False = False
    | otherwise = True
\end{code}
...
ppLine :: [Int] -> String
ppLine [] = "+"
ppLine (x:xs) = "+" ++ replicate x '-' ++ ppLine xs
\end{code}
...
ppField n [] = replicate n ' '
ppField n str = str ++ (replicate (n - length str) ' ')
\end{code}
...
ppRow [] = "|"
ppRow ((x,y):xs) = "|" ++ ppField x y ++ ppRow xs
\end{code}
...
maxValue :: Table -> [Int]
maxValue [] = []
maxValue (t:ts) = maximum (map length t) : maxValue ts

fieldSizes :: Table -> [Int]
fieldSizes t = maxValue (transpose t)
\end{code}
...
ppTable table = border ++ fields ++ border ++ instances ++ border where
    border = ppLine (fieldSizes table) ++ "\n"
    format row = ppRow (zip (fieldSizes table) row) ++ "\n"
    fields = format (head table)
    instances = concat (map format (tail table))
\end{code}
